...
Il s'est **inspiré** de : https://satijalab.org/seurat/articles/multimodal_vignette.html    
...
dir.create(path = paste0(directory_exp,"scRNAseq_CITEseq_ind_CreateSeuratObject/", current_date))
directory_output = paste0(directory_exp, "scRNAseq_CITEseq_ind_CreateSeuratObject/", current_date, "/")
...
create_seurat_ADT_RNA = function(matrix_file, barcode_file, feature_file) {
  ...
  # Create a global large dgCMatrix with genes, ADT (CITEseq) and HTO (multiplexing) information
  ...
  features = read.csv(paste0(directory_data, feature_file), sep = "\t", header = FALSE, col.names = c("id_gene", "name", "analysis"))
  ...
  tab_antibody = tab_matrix %>% dplyr::filter(row.names(tab_matrix) %in% features_antibody$name)
  tab_genes = tab_matrix %>% dplyr::filter(row.names(tab_matrix) %in% features_gene$name)
...
for (i in 1:number_condition) {
  ...
  list_seurat_obj[list_condition[i]] = create_seurat_ADT_RNA(
    matrix_file = str_subset(string = matrix_list, pattern = paste0(list_condition[i], "_")),
    barcode_file =  str_subset(string = barcodes_list, pattern = paste0(list_condition[i], "_")),
    feature_file = str_subset(string = features_list, pattern = paste0(list_condition[i], "_"))
  )
...
for (i in 1:number_condition) {
  ...
  cat(names(list_seurat_obj)[i], "- Assays :",
      Assays(list_seurat_obj[[i]]), " - CITEseq :",
      rownames(list_seurat_obj[[i]][["ADT"]]), "\n")
