...
list_seurat_obj = lapply(list_seurat_obj, function(x) { NormalizeData(object = x, 
                                                                      assay = "RNA",
...
list_seurat_obj = lapply(list_seurat_obj, function(x) { FindVariableFeatures(object = x, 
                                                                            assay = "RNA",
...
list_seurat_obj = lapply(list_seurat_obj, function(x) { ScaleData(object = x, 
                                                                  assay = "RNA",
                                                                  split.by = "orig.ident",
                                                                  verbose = TRUE) })
...
cat("There are", length(cc.genes$s.genes), "markers associated to S phase :", "\n", cc.genes$s.genes, "\n")
cat("There are", length(cc.genes$g2m.genes), "markers associated to G2M phase :", "\n", cc.genes$g2m.genes, "\n")
...
list_seurat_obj = lapply(list_seurat_obj, function(x) { CellCycleScoring(object = x, 
                                                                         assay = "RNA",
...
list_seurat_obj = lapply(list_seurat_obj, function(x) { RunPCA(object = x, 
                                                               assay = "RNA",
...
list_mat = lapply(list_seurat_obj, function(x) { as.data.frame(x@assays$RNA@counts) }) 

# Restriction des tableaux aux gènes du cycle cellulaire
print("Filter genes to keep only cell cycle genes")
list_mat = lapply(list_mat, function(x) { x %>% dplyr::filter(row.names(x) %in% c(cc.genes$s.genes, cc.genes$g2m.genes)) })
...
plot_smedian = ggplot(tab_cc %>% filter(phase == "S"), aes(x=gene, y=medianUMI, fill=condition)) +
  geom_col(position = "dodge") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
  #scale_fill_manual(values = color_code, limits = force) +
  ggtitle("Phase S genes - Median UMI")

plot_smean = ggplot(tab_cc %>% filter(phase == "S"), aes(x=gene, y=meanUMI, fill=condition)) +
    geom_col(position = "dodge") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
   #scale_fill_manual(values = color_code, limits = force) +
    ggtitle("Phase S genes - Mean UMI")
  
plot_g2median = ggplot(tab_cc %>% filter(phase == "G2M"), aes(x=gene, y=medianUMI, fill=condition)) +
    geom_col(position = "dodge") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
   #scale_fill_manual(values = color_code, limits = force) +
    ggtitle("Phase G2M genes - Median UMI")

plot_g2mean = ggplot(tab_cc %>% filter(phase == "G2M"), aes(x=gene, y=meanUMI, fill=condition)) +
    geom_col(position = "dodge") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
   #scale_fill_manual(values = color_code, limits = force) +
